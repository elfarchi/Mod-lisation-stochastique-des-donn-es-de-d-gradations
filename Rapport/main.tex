...
- \sum_{i=1}^m \sum_{j=1}^{n_i}
\Delta t_{i,j}\,
\psi(\hat a\Delta t_{i,j})
= 0,
\]
où $\psi(x)=\Gamma'(x)/\Gamma(x)$ désigne la fonction digamma.

\subsection{Comparaison des estimateurs}
L’application R Shiny permet de comparer les performances des estimateurs à partir de simulations de processus de dégradation. Sur un échantillon de 500 trajectoires simulées, les biais des estimateurs obtenus par la méthode des moments et par maximum de vraisemblance se révèlent très proches, aussi bien pour le processus de Wiener que pour le processus Gamma. Les biais sont évalués par une approche Monte Carlo, fondée sur la moyenne empirique des paramètres estimés sur l’ensemble des trajectoires simulées.
...
\qquad x>0.
\]
où $k_i = A(t_i) - A(t_{i-1}) = a\bigl(t_i^{c} - t_{i-1}^{c}\bigr).$

Par indépendance des accroissements, le log de la fonction de vraissemblance s'écrit :
\[
\ell(a,b,c)
=
\sum_{i=1}^{n}
...
\]
\noindent
Pour maximiser cette fonction on résoud le système : $\nabla \ell(a,b,c)=0,$ voir \cite{chatenet2021modeling} pour plus de détails.
...
\end{table}
La densité de probabilité du temps de défaillance n'admet pas d'expression simple.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.75\textwidth]{Recherche_Seuil_Optimal_Well_Made.pdf}
