...
\date{\today} %Projet Mathématiques Appliquées, \textit{Ensimag 1A},
...
\begin{abstract}
Ce rapport étudie la modélisation stochastique de la dégradation de systèmes industriels à l’aide des processus de Wiener et Gamma. Les paramètres des modèles sont estimés à partir de données observées par les méthodes des moments et du maximum de vraisemblance, pour une ou plusieurs trajectoires. Ces approches sont appliquées à des jeux de données réels et intégrées dans une application R Shiny permettant la simulation, l’estimation et la prévision de la défaillance. Des extensions vers des processus non homogènes et des politiques de maintenance imparfaite sont également abordées.
\end{abstract}
...
\begin{equation}
    X(t) = x_0 + \mu t + \sigma B_t,
    \label{eq:wiener}
\end{equation}
où $x_0$ représente l’état initial de dégradation, $\mu$ est le paramètre de dérive, $\sigma$ le paramètre de volatilité, et $(B_t)_{t \geq 0}$ un mouvement brownien standard. Dans la suite, on se place sans perte de généralité dans le cas $x_0 = 0$.
...
\begin{align}
& \ln(b)\sum_{i=1}^n \Delta t_i
+ \sum_{i=1}^n \Delta t_i \ln(\Delta x_i)
- \sum_{i=1}^n \Delta t_i\,\psi(a\Delta t_i) = 0,\\
...
\subsection{Comparasion des estimateurs}
L’application Shiny permet de simuler des processus de dégradation en faisant varier les paramètres du modèle. Les simulations du processus Gamma sur un échantillon de 500 trajectoires montrent que les estimateurs obtenus par maximum de vraisemblance présentent un biais significativement plus élevé que ceux issus de la méthode des moments.
Ces biais sont estimés par une approche Monte Carlo, basée sur la moyenne empirique des paramètres estimés sur l’ensemble des trajectoires simulées.
De la même manière, les simulations du processus de Weiner sur un même nombre de trajectoires montre que les biais pour les différentes méthodes sont très proches.
% -------------------------------------------------------------------
% -------------------------------------------------------------------
...
\end{equation}
on peut maximiser la log-vraisemblance en utilisant la fonction \texttt{minimize} du module \texttt{scipy.optimize} en Python (voir résultats dans le tableau ~\ref{tab:estimations_moments})

\textbf{Par la méthode des moments} : la procédure se déroule de la manière suivante :

\begin{enumerate}
\item On simule \(N\) trajectoires indépendantes
\(\{X_i(t),\, i=1,\dots,N\}\) du processus

\item À chaque instant \(t_j\), on estime l’espérance par la moyenne empirique :
\[
...
\begin{table}[h!]
    ...
    \begin{tabular}{cccc}
        \toprule
        Paramètre & Valeur réelle & Estimation(moment) & Estimation(vraissemblance) \\ 
        \midrule
...
La figure ~\ref{fig:time_to_failure} nous donne la superposition de la courbe théorique avec l'histogramme des temps de défaillance déterminé par simulation de plusieurs trajectoires après estimation des paramètres.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth,height = 0.3\textheight]{histograrmme des temps de defaillancec.pdf}
    \caption{Histogramme des temps de défaillance des trajectoires simulées avec la densité théorique.}
...
où  $A(t) = at^c$ ;  t > 0 : la fonction de forme,   
    $a ,c$ > 0,      
   et  $b$ : le paramètre de taux \\
    

\subsubsection{Calcul de la fonction de vraisemblance}
Pour ce qui est de l'estimation des paramètres on procède cette fois par maximum de vraissemblance.\\ 
...
\end{table}
La densité de probabilité du temps de défaillance n'admet pas d'expression simple et ne sera determinée que par simulation de plusieurs trajectoires après estimation des paramètres.
% -------------------------------------------------------------------
...
La dégradation du système est modélisée par un processus de Wiener avec dérive :
\[
X(t) = \mu t + \sigma B(t),
\]
où $\mu > 0$ représente la vitesse moyenne de dégradation, $\sigma > 0$ l'intensité des fluctuations aléatoires, et $B(t)$ un mouvement brownien standard. La défaillance du système survient lorsque le niveau de dégradation atteint un seuil critique $L$, définissant un temps de défaillance
\[
T_f = \inf \{ t \ge 0 : X(t) \ge L \}.
\]
Ce modèle est couramment utilisé pour représenter des mécanismes d'usure progressive soumis à des incertitudes environnementales.

\subsection{Maintenance imparfaite par réduction de l'état}
La maintenance est supposée imparfaite : une intervention ne remet pas le système à l'état neuf, mais réduit partiellement son niveau de dégradation. Plus précisément, lorsqu'une maintenance est déclenchée au franchissement d'un seuil de sécurité $h < L$, le niveau de dégradation est instantanément réduit selon
\[
X(\tau^+) = \alpha X(\tau^-), \quad 0 < \alpha < 1,
\]
où $\alpha$ mesure l'efficacité de la maintenance.

La politique étudiée est une politique à seuil : tant que $X(t) < h$, aucune intervention n'est effectuée ; dès que $X(t)$ atteint $h$, une maintenance est réalisée. Le remplacement du système n'intervient qu'à la défaillance, lorsque $X(t) \ge L$. La dernière maintenance est ainsi définie implicitement comme la dernière intervention avant l'atteinte du seuil de défaillance. 

Il est important de souligner que la maintenance déclenchée au seuil $h$ n'empêche pas la défaillance du système. En raison de la composante aléatoire du processus de Wiener, le niveau de dégradation peut franchir simultanément les seuils $h$ et $L$ entre deux instants, conduisant à une défaillance avant toute intervention. L'intensité de ce phénomène est contrôlée par le paramètre $\sigma$, qui mesure l'importance des fluctuations aléatoires autour de la tendance moyenne de dégradation.

%En raison de la composante aléatoire du processus de Wiener, la maintenance à seuil n'empêche pas la défaillance, un franchissement direct du seuil critique pouvant survenir avant toute intervention.
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{Recherche_Seuil_Optimal_Well_Made.pdf}
